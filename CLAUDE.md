...
- **Démarrage sur l'exemple** (par défaut, en l'absence de sauvegarde locale) : `voyage-colomban.json` (relevé de Colomban pré-enrichi, legs + temps de mer bakés) est chargé d'emblée, sans routeur. searoute ne se charge qu'à la 1re édition. Le bouton « Charger l'exemple » le recharge à la demande ; « Réinitialiser » revient au vierge (l'exemple revient au prochain chargement). **Défauts de l'affiche** : titre = clé i18n `default_title` (« Mes embarquements » / « My sea service », repli localisé si le titre est vide, le nom du marin reste un repli secondaire dans `render.js`) ; style = `nautical` (Carte marine) + projection `natural` (Natural Earth), soit le 2e preset.
